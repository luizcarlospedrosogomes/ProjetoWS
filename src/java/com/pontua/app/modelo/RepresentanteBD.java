...
import com.pontua.app.objeto.Representante;
...
public class RepresentanteBD {
    ...
    public boolean loginRepresentante(Representante r){
        try {
            sql = "SELECT * FROM representante WHERE email = ? AND senha = ?;";
            con = Conexao.conectar();
            pst = con.prepareStatement(sql);
            
            pst.setString(1,r.getEmail());
            pst.setString(2,r.getSenha());
